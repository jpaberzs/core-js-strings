...
function removeFirstOccurrences(str, value) {
  const strIndex = str.indexOf(value);
  if (str.includes(value)) {
    return str.slice(0, strIndex) + str.slice(value.length + strIndex);
  }
  return str;
}
...
function isPalindrome(str) {
  ...
  const reversStr = lowRegStr.split('').reverse().join('');
  return lowRegStr === reversStr;
}
...
function encodeToRot13(str) {
  ...
  //   encode += code[alp.indexOf(str[i])];
  // }
  return str
    .split('')
    .map((item) => {
      if (item === ' ' || item === '?' || item === '!') return item;
      return code[alp.indexOf(item)];
    })
    .join('');
  // return encode;
}
